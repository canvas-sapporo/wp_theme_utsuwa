...
const Header: React.FC = () => {
  ...
  return (
    <>
      <header className="w-full min-w-0 bg-light h-[58px] lg:h-20 z-20 border-dark">
        <nav className="container mx-auto h-full lg:my-5 lg:h-auto flex items-center justify-between gap-4 lg:gap-8 px-4">
          <button
            type="button"
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0">
            <img src={logo} alt="Utsuwa Logo" className="h-8 lg:h-10" />
            <span className="text-logo font-bold text-2xl lg:text-4xl">
              Utsuwa
            </span>
          </Link>
          <div className="hidden lg:flex gap-8 text-cta shrink-0">
      ...
      {/* モバイル用サイドバー */}
      <aside
        className="lg:hidden fixed left-0 top-0 bottom-0 w-64 bg-light border-r border-dark z-40 flex flex-col transition-transform duration-300 ease-out"
        style={{
